...
class ReportBomStructure(models.AbstractModel):
    ...
    def get_warehouses(self):
        return self.env["stock.warehouse"].search_read(
            [("company_id", "in", self.env.companies.ids)], fields=["id", "name"]
        )
    ...
    def _compute_current_production_capacity(self, bom_data):
        ...
        for comp in bom_data.get("components", []):
            if comp["product"].type != "product" or float_is_zero(
                comp["base_bom_line_qty"], precision_digits=comp["uom"].rounding
            ):
                continue
    ...
    def _compute_production_capacities(self, bom_qty, bom_data):
        ...
        if bom_data.get("producible_qty", 0):
            ...
            res["earliest_date"] = format_date(
                self.env, date_today + timedelta(days=lead_time)
            )
    ...
    def _get_components_closest_forecasted(
        ...
        for line in lines:
            ...
            product_info[product.id]["consumptions"][stock_loc] += line_quantities.get(
                line.id, 0.0
            )
    ...
    def _get_bom_data(
        ...
        for line in bom.bom_line_ids:
            ...
            product_line = self.env["product.product"].browse(line.product_id.id)
            line_barcode = (
                product_line.product_tmpl_id.barcode
                if product_line.product_tmpl_id
                else "No Barcode"
            )
    ...
    def _get_component_data(
        ...
        # Fetch the barcode
        barcode = (
            bom_line.product_id.product_tmpl_id.barcode
            if bom_line.product_id.product_tmpl_id
            else "No Barcode"
        )
    ...
    def _get_quantities_info(
        ...
        quantities_info = {
            "free_qty": (
                max(product.uom_id._compute_quantity(product.free_qty, bom_uom), 0)
                if product.detailed_type == "product"
                else 0
            ),
            "on_hand_qty": (
                product.uom_id._compute_quantity(product.qty_available, bom_uom)
                if product.detailed_type == "product"
                else 0
            ),
            "stock_loc": "in_stock",
    ...
    def _get_pdf_line(
        ...
        barcode = product.product_tmpl_id.barcode
        if self.env.context.get("warehouse"):
            warehouse = self.env["stock.warehouse"].browse(
                self.env.context.get("warehouse")
            )
    ...
    def _get_bom_array_lines(
        ...
        for bom_line in bom_lines:
            ...
            product_record = self.env["product.product"].browse(bom_line["product_id"])
            barcode = (
                product_record.product_tmpl_id.barcode
                if product_record.product_tmpl_id
                else ""
            )
            lines.append(
                {
                    ...
                    "barcode": bom_line.get("barcode", "No Barcode"),
    ...
    def _get_resupply_route_info(
        ...
        if self._need_special_rules(product_info, parent_bom, parent_product):
            found_rules = self._find_special_rules(
                product, product_info, parent_bom, parent_product
            )
    ...
    def _find_special_rules(
        self, product, product_info, parent_bom=False, parent_product=False
    ):
        return False
    ...
    def _format_route_info(self, rules, rules_delay, warehouse, product, bom, quantity):
        ...
        if manufacture_rules:
            ...
            return {
                ...
                "lead_time": bom.produce_delay
                + rules_delay
                + manufacturing_lead
                + bom.days_to_prepare_mo,
